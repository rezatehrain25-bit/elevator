...
namespace Injector {
    ...
    void PayloadInjector::Inject(const std::wstring& pipeName) {
        ...
        m_console.Debug("Creating remote thread (SUSPENDED) via syscall...");
        // CREATE_SUSPENDED = 0x00000004 (6th parameter = TRUE means create suspended)
        ...
        if (status != 0) {
            throw std::runtime_error("Thread creation failed with status: " + std::to_string(status));
        }

        ss.str("");
        ss << "  [+] Thread created in SUSPENDED state (entry: 0x" << std::hex << entry << ")";
        m_console.Debug(ss.str());
    }

    void PayloadInjector::ResumeRemoteThread() {
        if (!m_remoteThread || m_remoteThread == INVALID_HANDLE_VALUE) {
            throw std::runtime_error("Remote thread handle is invalid");
        }
        ...
        if (status != 0) {
            throw std::runtime_error("NtResumeThread failed with status: " + std::to_string(status));
        }

        std::stringstream ss;
        ss << "  [+] Payload thread resumed (suspend count was: " << suspendCount << ")";
        m_console.Debug(ss.str());
    }
